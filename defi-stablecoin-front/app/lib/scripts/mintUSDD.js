...
export default async function mintUSDD(amountUSDDToMint) {
  ...
  try {
    ...
    const tx = await dscEngine.mintUSDD(amountUSDDToMint);
    await tx.wait();

    console.log("Success to mint USDD__ScriptMintUSDD");
  } catch (error) {
    console.error("Error to mint USDD__ScriptMintUSDD: ", error);
  }
}
